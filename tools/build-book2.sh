#!/usr/bin/env bash
# build-book2.sh — 第二冊組建管線：md → (mermaid→PNG) → PDF(typst) + EPUB
# 用法：bash tools/build-book1.sh
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
DRAFTS="$ROOT/book2-practical/drafts"
BUILD="$ROOT/build/book2"
ASSETS="$BUILD/assets"
...
pandoc "$BUILD/manuscript-rendered.md" \
  -o "$BUILD/OpenCode實戰寶典-draft.pdf" \
  --pdf-engine=typst \
...
echo "== 4/4 產出 EPUB =="
pandoc "$BUILD/manuscript-rendered.md" \
  -o "$BUILD/OpenCode實戰寶典-draft.epub" \
  -f markdown-citations \
  --split-level=1 \
  --css="$CSS" \
  --toc --toc-depth=2 \
