...
#place(top + left, dx: 1.9cm, dy: 2.2cm)[
  #box(stroke: 1pt + green.transparentize(45%), radius: 999pt, inset: (x: 16pt, y: 7pt), fill: green.transparentize(93%))[
    #text(font: "DejaVu Sans Mono", size: 11pt, tracking: 1pt, fill: green)[OPENCODE 技術叢書 / 02]
  ]
]
...
#place(top + left, dx: 1.85cm, dy: 5.7cm)[
  ...
  #v(0.45cm)
  #text(weight: 900, stroke: 0.6pt + white, size: 82pt, tracking: 5pt, fill: white)[實戰寶典]

  #v(0.75cm)
  #line(length: 1.7cm, stroke: 2.5pt + gradient.linear(angle: 90deg, green, cyan))
  #h(0.35cm)
  #text(size: 15.5pt, tracking: 1.5pt, fill: dim)[從概念到實踐的完整學習路徑]
]
...
#place(bottom + left, dx: 1.85cm, dy: -6.6cm)[
  #block(width: 12.3cm, fill: rgb("#0F172A"), stroke: 0.9pt + rgb("#94A3B8").transparentize(72%), radius: 10pt, clip: true)[
    ...
    #block(inset: (x: 16pt, y: 12pt))[
      ...
      #text(fill: green)[\$] opencode run --agent refactor\
      #text(fill: cyan)[>] 把 auth 模組重構成服務層\
      #text(fill: faint)[● 計畫：拆分五檔 · 補測試 · 分步提交]\
      #text(fill: faint)[● 執行：編輯 → 測試 → 修復迴圈 ×3]\
      #text(fill: amber)[測試 42 通過。]#text(fill: green)[重構完成，已分步提交。]
    ]
  ]
]
...
#place(bottom + left, dy: -1.55cm, dx: 1.85cm)[
  ...
  #grid(columns: (auto, 1fr), column-gutter: 0pt,
    ...
    align(right)[#text(font: "DejaVu Sans Mono", size: 8.5pt, tracking: 1pt, fill: faint)[OPENCODE v1.X · 2026\ EXPLORE / DEBUG / SHIP]]
  )
]
]
