...
#place(top + left, dx: 1.9cm, dy: 2.2cm)[
  #box(stroke: 1pt + green.transparentize(45%), radius: 999pt, inset: (x: 16pt, y: 7pt), fill: green.transparentize(93%))[
    #text(font: "DejaVu Sans Mono", size: 11pt, tracking: 1pt, fill: green)[OPENCODE 技術叢書 / 05]
  ]
]
...
#place(top + left, dx: 1.85cm, dy: 4.3cm)[
  ...
  #v(0.45cm)
  #text(weight: 900, stroke: 0.6pt + white, size: 82pt, tracking: 5pt, fill: white)[團隊作戰]

  #v(0.75cm)
  #line(length: 1.7cm, stroke: 2.5pt + gradient.linear(angle: 90deg, green, cyan))
  #h(0.35cm)
  #text(size: 15.5pt, tracking: 1.5pt, fill: dim)[從個人效率到組織級 AI 工程化]
]
...
#place(bottom + left, dx: 1.85cm, dy: -5.4cm)[
  #block(width: 12.3cm, fill: rgb("#0F172A"), stroke: 0.9pt + rgb("#94A3B8").transparentize(72%), radius: 10pt, clip: true)[
    ...
    #block(inset: (x: 16pt, y: 12pt))[
      ...
      #text(fill: green)[\$] opencode github install\
      #text(fill: cyan)[&gt;] App＋workflow＋secrets——就緒\
      #text(fill: faint)[● issue 留言 /oc → 代理開分支提 PR]\
      #text(fill: amber)[六種事件。]#text(fill: green)[團隊，集結。]
    ]
  ]
]
...
#place(bottom + left, dy: -1.55cm, dx: 1.85cm)[
  ...
  #grid(columns: (auto, 1fr), column-gutter: 0pt,
    ...
    align(right)[#text(font: "DejaVu Sans Mono", size: 8.5pt, tracking: 1pt, fill: faint)[OPENCODE v1.X · 2026\ SHARE / AUTOMATE / GOVERN]]
  )
]
]
